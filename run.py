...
l2_regularizer_coeff_list = [0.0,0.01,0.1,0.5]
...
for num_layers,fea,num_sequence,kmeans_k,k,conv_filter_size,l2_regularizer_coeff in tqdm(all_comb,total=length):
	exp_name = "mnist_numLayers_{:}_fea_{:}_numSeq_{:}_kmeansK_{:}_k_{:}_convFilterSize_{:}_l2RegularizerCoeff_{:}".format(num_layers,fea,num_sequence,k,k,conv_filter_size,l2_regularizer_coeff)
	out_dir = './out'+'/'+exp_name
	summary_dir = './summary_dir' + '/' + exp_name
	pic_dir = './pic'+'/'+exp_name

	desc = "mnist training (0-5) numLayers {:} fea {:} numSeq {:} kmeansK {:} k {:} conv_filter_size{:} l2_regularizer_coeff {:}".format(num_layers,fea,num_sequence,k,k,conv_filter_size,l2_regularizer_coeff)
