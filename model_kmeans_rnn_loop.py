...
class MetaCluster():
	...
	def model(self):
		...
		with tf.variable_scope('core'):
			...
			output_two, states = tf.nn.bidirectional_dynamic_rnn(fw_cell, bw_cell, sequences, dtype=tf.float32, initial_state_fw = cell_init_state_fw,  initial_state_bw = cell_init_state_bw)
	...
	def train(self,data,labels,centriod,sess):
		...
		for epoch_ind in range(30):
			...
			data = data[:,perm,:]
			labels = labels[:,perm]
			_, summary, _,loss = sess.run([model.keep_state_op, model.merged,model.opt,model.loss],feed_dict={model.sequences:data,model.labels:labels,model.centriod:centriod})
	...
	def test(self,data,labels,sess,real_centriods=None,validation=False,centriods=None):
		...
		if validation:
			...
			for epoch_ind in range(30):
				...
				data = data[:,perm,:]
				labels = labels[:,perm]
				_,summary,centriods,loss = sess.run([model.keep_state_op,model.merged,model.predicted_centroids_reshape,model.loss],feed_dict={model.sequences:data,model.labels:labels,model.centriod:centriods})
